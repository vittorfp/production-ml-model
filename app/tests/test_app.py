...
def test_input_completeness_3(client):
    """ Missing lat """
    response = client.get('/predict', query_string={'lon': -65})
    assert response.status_code == 422


def test_input_completeness_4(client):
    """ Happy case (every parameter present) """
    response = client.get('/predict', query_string={'lon': -65, 'lat': -77})
    assert response.status_code != 422


def test_input_boundary_1(client):
    """ Lat out of bounds (lower) """
    response = client.get('/predict', query_string={'lon': -65, 'lat': -90.1})
    assert response.status_code == 422
...
def test_input_boundary_3(client):
    ...
    response = client.get('/predict', query_string={'lon': -180.1, 'lat': -77})
    assert response.status_code == 422
...
def test_input_boundary_5(client):
    ...
    response = client.get('/predict', query_string={'lon': -65, 'lat': 90})
    assert response.status_code != 422
...
def test_input_boundary_8(client):
    ...
    response = client.get('/predict', query_string={'lon': -180, 'lat': -77})
    assert response.status_code != 422
